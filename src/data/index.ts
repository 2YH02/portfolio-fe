    ...
    sections: [
      {
        ...
        bullets: [
          `서버에서 5,000개 이상의 위치 데이터를 받아 카카오 지도 API를 통해 마커를 표시했을 때, 클러스터러 
적용 시 지도를 이동하거나 확대/축소할 때마다 CPU 점유율이 100%를 넘기며 화면이 일시적으로
멈추는 문제가 발생했습니다. 이를 개선하기 위해 지도의 중심을 기준으로 특정 범위 내의 마커만 연산·표시하도록 
처리하고, Grid-Based 클러스터링 알고리즘을 사용해 클러스터러를 직접 구현하여 카카오 
지도 API의 클러스터러를 대체했습니다. 그 결과 CPU 점유율이 최대 30% 수준으로 안정화되었고,
사용자 경험도 크게 향상되었습니다.`,
        ],
      },
      {
...
식별한 뒤 PWA 설치를 유도하는 안내 창을 노출하도록 구현했습니다. 이후 프로젝트를 Next.js로 전환하면서 
기존 PWA는 제거하고, React Native Expo 기반의 WebView 앱을 도입하였습니다.`,
        ],
      },
      {
        title: "이미지 용량 감소를 위한 접근",
        bullets: [
          `서비스에 거리뷰를 기능을 도입하면서 기존 정보로 제공하던 이미지의 해상도의 중요도가 낮아졌다고 
팀끼리 판단하였습니다. 따라서 이미지 해상도 감소는 감안하고 서비스의 로딩 속도와 사용자
경험을 위해 이미지 용량을 줄이는 전략을 채택했습니다. JavaScript의 Canvas API를 활용하여 사용자가 
업로드하는 이미지의 크기를 실시간으로 80% 줄이는 기능을 구현했습니다. 이 과정은 서버로의
...
Zustand 라이브러리를 활용한 전역 상태 관리를 도입했습니다. 사용자가 아이템을 클릭하는 순간을 
전역 상태로 관리하여, 페이지 데이터 로딩 동안 즉각적으로 스켈레톤 UI를 표시함으로써 사용자
경험을 개선했습니다.`,
        ],
      },
      {
        ...
        bullets: [
          `Zustand의 persist 플러그인을 활용하여 무한 스크롤로 구현된 아이템 리스트에서 사용자의 스크롤 
위치를 저장하고 관리했습니다. 이를 통해 사용자가 아이템 상세 페이지를 방문한 후 이전 페이지로 
돌아올 때 이전에 보았던 위치로 자동으로 스크롤이 복원되도록 구현하여, 사용자가 정보를 찾는데 
있어 연속성과 편리성을 개선했습니다.`,
        ],
      },
      {
...
시점의 URL을 저장했습니다. 이후 사용자가 로그인이나 회원가입을 완료하면 자동으로 이전에 머물렀던 
페이지로 리다이렉트 되도록 구현함으로써, 사용자의 흐름을 자연스럽게 유지시켜 주었습니다.`,
        ],
      },
      {
...
데이터를 사전에 불러와 초기 로딩 시간을 줄이고, 결과적으로 서비스의 성능을 향상시키는 데 기여했습니다. 
이러한 기술적 접근을 통해 사용자에게 더 빠르고 부드러운 페이지 전환 경험을 제공하며,
...
설정하여 인증이 필요한 경우에 인증되지 않은 사용자가 접근 시 자동으로 로그인 페이지로 이동하는 
모달창이 공통적으로 표시되도록 구현했습니다. 이는 페이지별로 모달창을 관리하며 발생했던 코드의 
복잡도를 줄이고, 사용자 경험을 간결하고 일관되게 유지하는 데 기여했습니다.`,
        ],
      },
      {
        title: "장바구니 인터페이스 개선",
        bullets: [
          `사용자가 장바구니에 다수의 상품을 담았을 때, 예상 구매 내역이 스크롤 시 화면에서 사라지는 문제를 
해결하기 위해 CSS의 position 속성에 sticky 값을 적용했습니다. 이로 인해, 사용자가 페이지를
아래로 스크롤해도 예상 구매 내역이 화면 상단에 고정되어 지속적으로 표시되도록 함으로써, 사용자의 
편의성을 개선하고 구매 결정 과정을 더욱 효율적으로 만들었습니다.`,
        ],
      },
      {
        title: "반응형 디자인 구현",
        bullets: [
          `Media Query를 활용한 반응형 디자인을 추가하여, 사용자가 사용 중인 장치의 화면 너비에 따라 UI가 
다르게 보이도록 개선했습니다. 이는 다양한 디바이스에서의 사용자 경험을 최적화하며, 모든 사용자에게 
일관된 서비스 접근성을 제공했습니다.`,
        ],
      },
      {
        ...
        bullets: [
          `Geolocation API를 사용하여 사용자의 위치 정보를 활용, 맞춤형 숙소 정보를 제공함으로써 서비스의 
편의성과 정확성을 높였습니다. 이 기술은 사용자에게 현재 위치에 기반한 개인화된 추천을 가능하게 하여, 
서비스 이용 시의 만족도가 향상 되도록 하였습니다.`,
        ],
      },
    ],
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          "이미지 압축 시 품질 저하를 최소화하기 위해 Rust WebAssembly를 이용하여 기존 JavaScript 구현보다 더 효율적이고 높은 퀄리티의 이미지 압축을 할 수 있도록 추가로 구현했습니다.",
        ],
      },
    ],
    reflection:
